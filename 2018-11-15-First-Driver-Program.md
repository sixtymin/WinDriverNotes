...
驱动的设备对象由驱动对象结构体中的`DeivceObject`字段指向，一个驱动对应只有一个驱动对象，它可以创建多个设备对象，设备对象之间由设备对象结构体的`NextDevice`字段形成链表，这样形成的是设备对象的水平结构。设备对象创建时设置名字，大多以`\Device\XXXXXX`形式的命名，要在应用层访问驱动无法直接使用设备名，需要为设备对象设置一个符号链接，符号链接都是以`\??\YYYY`或`\DosDevices\YYYY`或`\Global??\YYYY`形式命名。在用户层则需要以`\\.\YYYY`或`\\.\Global\YYYY`形式来打开驱动并使用句柄访问驱动。

> 其中`\DosDevices`其实是`\??`的符号链接，内核解析`\??`会被转换为`\Global??`。而用户层的访问方式中，`\\.\`其实就是`\\Global??`，而`\\.\Global`其实是`\\Global??`的符号链接。

符号链接其实是有很多缺点的，符号名字重名不好处理；再者符号链接也没办法对驱动类型和设备对象进行分类。使用设备接口就可以避免这些问题。
...
设备对象是系统定义的内核对象，它是驱动程序的通讯接口，系统模块或用户程序通过设备对象和驱动模块通信。按照设备对象的作用可以分为如下的集中类型：
